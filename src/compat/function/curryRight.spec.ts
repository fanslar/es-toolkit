...
describe('curryRight', () => {
  ...
  it('should use `this` binding of function', () => {
    ...
    expect(bind(curryRight(fn), object)('c')('b')('a')).toEqual(Array.from({ length: 3 }))
    expect(bind(curryRight(fn), object)('b', 'c')('a')).toEqual(Array.from({ length: 3 }))
  ...
  it('should work with partialed methods', () => {
    ...
    const b = partialRight(a, 3) as any
    const c = bind(b, null, 1)
    const d = partial(b(2), 1) as any
  ...
  it(`\`curryRight\` should work for function names that shadow those on \`Object.prototype\``, () => {
    const curried = curryRight((a: unknown, b: unknown) => {
      return [a, b]
    })
