...
describe('clamp', () => {
  ...
  it('should handle special NaN combinations', () => {
    expect(clamp(5, Number.NaN, Number.NaN)).toBe(0)
    expect(clamp(Number.NaN, Number.NaN, 10)).toBe(Number.NaN)
    expect(clamp(5, Number.NaN, undefined as any)).toBe(0)
    expect(clamp(5, undefined as any, Number.NaN)).toBe(0)
  })
