...
```typescript
import { unary } from '@fanslar/es-toolkit/function'
...
console.log(numbers.map(Number.parseInt)) // [1, NaN, NaN, NaN, NaN] (unexpected result)

// Use unary to pass only the first parameter
console.log(numbers.map(unary(Number.parseInt))) // [1, 2, 3, 4, 5] (expected result)
