...
```typescript
import { unary } from '@fanslar/es-toolkit/compat'
...
const numbers = ['1', '2', '3']
numbers.map(Number.parseInt) // [1, NaN, NaN] - unexpected result
numbers.map(unary(Number.parseInt)) // [1, 2, 3] - correct result
```
