...
```typescript
import { ary } from '@fanslar/es-toolkit/compat'
...
console.log(numbers.map(Number.parseInt)) // [1, NaN, NaN, NaN, NaN]

// Use ary to pass only the first argument
console.log(numbers.map(ary(Number.parseInt, 1))) // [1, 2, 3, 4, 5]
```
...
console.log(ary(func, Number.NaN)(1, 2, 3)) // [] (NaN treated as 0)
```
